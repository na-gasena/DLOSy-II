...
class DrawingMode {
  ...
  // When on (with AUTO CYCLE), the slot sequence restarts at slot 0 on the VCO
  // loop head (sequencer step 0) so the drawing cycle stays locked to the loop.
  loopSync: boolean = false;
    ...
    container.innerHTML = `
      <div class="panel-title">
        ...
        <div class="draw-header-controls">
          ...
          <button id="draw-loop-sync" class="small-btn" title="AUTO CYCLE をVCOループ先頭(step0)にリセット同期">LOOP SYNC</button>
      ...
      <div class="draw-lissajous-row" id="draw-lissajous-row">
        <span class="draw-wave-label">LISSAJOUS</span>
        <button class="small-btn" data-lj="1,1,1.5708" title="円 (1:1, 90°)">○</button>
        <button class="small-btn" data-lj="1,2,0" title="8の字 (1:2)">∞</button>
        <button class="small-btn" data-lj="3,2,1.5708" title="3:2">3:2</button>
        <button class="small-btn" data-lj="3,4,1.5708" title="3:4">3:4</button>
        <button class="small-btn" data-lj="5,4,1.5708" title="5:4">5:4</button>
        <button class="small-btn" data-lj="1,1,0" title="斜線 (1:1, 0°)">／</button>
      </div>
  ...
  bindControls() {
    ...
    // LOOP SYNC toggle — restart the auto-cycle at slot 0 on the VCO loop head.
    document.getElementById('draw-loop-sync')?.addEventListener('click', () => {
      this.loopSync = !this.loopSync;
      document.getElementById('draw-loop-sync')?.classList.toggle('vco-on', this.loopSync);
    });

    // Lissajous presets: load a parametric figure into the active slot.
    document.getElementById('draw-lissajous-row')?.addEventListener('click', (e) => {
      const btn = (e.target as HTMLElement).closest('[data-lj]') as HTMLElement | null;
      if (!btn) return;
      const [a, b, d] = (btn.dataset.lj || '').split(',').map(Number);
      this.lissajousPreset(a, b, d);
    });
  ...
  // Advance to next slot (called from the transport on each step tick). When
  // LOOP SYNC is on, the sequence restarts at slot 0 on the loop head (step 0) so
  // the drawing cycle stays locked to the VCO loop instead of free-running.
  advanceSlot(stepIndex?: number) {
    if (!this.autoSlotCycle) return;
    if (this.loopSync && stepIndex === 0) {
      if (this.activeSlot !== 0) this.switchSlot(0);
      return;
    }
  ...
  // Load a ready-made point path into the active slot and finalize it (redraw,
  // convert to waveform, refresh the live oscillator) — shared by the presets.
  _loadPresetPoints(points: DrawPoint[]) {
    if (points.length < 2) return;
    this.slots[this.activeSlot].points = points;
    this.redrawCanvas();
    this.convertToWaveform();
    this.updateWaveformPreview();
    this.updatePointCount();
    if (vcoLoop && vcoLoop.isOscRunning && vcoLoop.waveType === 'drawing') vcoLoop.refreshDrawingOsc();
    if (arpeggiator && arpeggiator.refreshDrawingOsc) arpeggiator.refreshDrawingOsc();
  }

  // Generate a Lissajous figure into the active slot:
  //   x = sin(a·t + δ),  y = sin(b·t),  t ∈ [0, 2π]
  // a:b is the frequency ratio, δ the phase offset (radians).
  lissajousPreset(a: number, b: number, delta: number) {
    ...
    for (let i = 0; i <= N; i++) {
      ...
      pts.push({
        x: cx + R * Math.sin(a * t + (delta || 0)),
        y: cy + R * Math.sin(b * t),
      });
    }
    this._loadPresetPoints(pts);
  }
